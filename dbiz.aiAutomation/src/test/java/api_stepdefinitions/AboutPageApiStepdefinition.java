...
public class AboutPageApiStepdefinition {

	private Response response;
	private static final Logger logger = LogManager.getLogger(AboutPageApiStepdefinition.class);

	@Given("user calls AboutPageAPI with GET http request")
	public void userCallsAboutUsAPIWithGETRequest() {
		try {
			// Construct the base URI by formatting the baseURL with the environment
			// property
			String baseURI = String.format(PropertiesFile.getProperty("AboutPageURI"),
			...
			logger.info("GET request to AboutUsAPI has been sent successfully.");
		} catch (Exception e) {
			logger.error("An error occurred while sending the GET request to AboutUsAPI: " + e.getMessage());
			throw e;
		}
	}

	@Then("the AboutPageAPI call got success with status code {int}")
	...
	@Then("Validating response body from the AboutPageAPI")
	public void validatingResponseBodyFromAboutUsAPI() {
		try {
			...
			response.then().log().all() // Log response details (including headers and body)
					...
					.body("nid", equalTo("2")) // Assert the 'nid' field
					.body("uuid", not(emptyOrNullString())) // Assert the 'uuid' field is not empty
					.body("title", equalTo("About")); // Assert the 'title' field
